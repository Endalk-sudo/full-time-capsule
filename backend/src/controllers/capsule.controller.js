...
import {
  createCapsuleSchema,
  capsuleUrlParamSchema,
} from "../validations/capsule.validation.js";
...
export const deleteCapsule = async (req, res) => {
  ...
  try {
    ...
    if (capsule.status === "SENT") {
      return res
        .status(400)
        .json({ message: "capsule is already sent you can't delete it" });
    }
    ...
    res.json({ message: "delete capsule", result });
  } catch (error) {
    res.status(500).json({ error: "Internal Server Error" });
  }
};
